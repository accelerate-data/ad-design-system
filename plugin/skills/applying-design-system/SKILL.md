...
## Runnable design-system bundle

Alongside the guidance below, this skill ships a **Claude-designed, runnable** design system — generated by Claude from the brand book, logo asset tree, and reference surfaces. Use it instead of re-deriving tokens by hand:

- **`README.md`** — the full design guide: brand context, content voice, visual foundations, iconography, and a manifest of everything here. Read it first when building.
- **`styles.css`** — the single stylesheet to link; it `@import`s every token and font file.
